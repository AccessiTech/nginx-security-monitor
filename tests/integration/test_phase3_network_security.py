...
class TestNetworkSecurityIntegration(BaseIntegrationTest):
    ...
    def test_network_threat_detection_flow(self):
        ...
        with patch.object(network_security, "check_ip_reputation") as mock_reputation:
            ...
            malicious_threats = [
                t for t in processed_threats if t["source_ip"] == "203.0.113.42"
            ]
    ...
    def test_ip_reputation_integration(self):
        ...
        network_security.configure(
            {
                "reputation_thresholds": {"suspicious": 50, "malicious": 80},
                "enable_ip_reputation": True,
        ...
        suspicious_result = network_security.check_ip_reputation(
            "198.51.100.50"
        )  # Use test IP from db
    ...
    def test_geographic_blocking_integration(self):
        ...
        network_security.configure(
            {
                "blocked_countries": [
                    "XA",
                    "XB",
                ],  # Match the test geo data country codes
        ...
        blocked_ip = (
            "203.0.113.50"  # Should be from blocked country XA (from test geo db)
        )
        ...
        with patch.object(mitigation, "block_ip") as mock_block_ip:
            ...
            mock_block_ip.assert_called_once_with(
                blocked_ip, reason="Country blocked: XA"
            )
            ...
            mock_block_ip.assert_called_once_with(
                "198.51.100.50", reason="Country blocked: XB"
            )
