...
def run_test_phase(phase_name, test_modules, verbosity=2):
    ...
    for module_name in test_modules:
        ...
        print(
            f"📊 Results: {result.testsRun} tests, {len(result.failures)} failures, {len(result.errors)} errors"
        )
...
def generate_report(all_results):
    ...
    with open(report_file, "w") as f:
        ...
        total_modules = sum(
            len(phase_results) for phase_results in all_results.values()
        )
        total_tests = sum(
            sum(result.testsRun for _, result in phase_results)
            for phase_results in all_results.values()
        )
        total_failures = sum(
            sum(len(result.failures) for _, result in phase_results)
            for phase_results in all_results.values()
        )
        total_errors = sum(
            sum(len(result.errors) for _, result in phase_results)
            for phase_results in all_results.values()
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run NGINX Security Monitor Integration Tests"
    )
    parser.add_argument(
        "--phase", type=str, help="Run only a specific phase (phase1, phase2, phase3)"
    )
    parser.add_argument(
    ...
    if args.phase:
        ...
        if args.phase in test_phases:
            ...
            results = run_test_phase(
                phase_info["name"], phase_info["modules"], args.verbose
            )
    ...
    total_failures = sum(
        sum(len(result.failures) + len(result.errors) for _, result in phase_results)
        for phase_results in all_results.values()
    )
