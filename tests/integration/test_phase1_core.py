...
class TestThreatDetectionPipelineIntegration(BaseIntegrationTest):
    ...
    def test_complete_threat_detection_pipeline(self):
        ...
        for log_entry in test_logs:
            try:
                ...
                self.assertIsNotNone(
                    parsed_data, "Log parser should return structured data"
                )

                # Step 2: Log Processor validates and enriches data
                processed_data = self.components["log_processor"].process_log_entry(
                    parsed_data
                )
                if processed_data:
                    ...
                    patterns = self.components["pattern_detector"].detect_patterns(
                        [processed_data]
                    )
                    ...
                    if patterns:
                        for pattern in patterns:
                            threat = self.components["threat_processor"].process_threat(
                                pattern
                            )
                            if threat:
        ...
        self.assertGreater(
            len(processed_logs), 0, "Should process some logs successfully"
        )
        ...
        for expected_type in expected_types:
            self.assertIn(
                expected_type, threat_types, f"Should detect {expected_type} threats"
            )

        print(
            f"✅ Pipeline processed {len(processed_logs)} logs, detected {len(threats_detected)} threats"
        )
    ...
    def test_malformed_log_handling_across_pipeline(self):
        ...
        self.assertEqual(
            successful_processing + errors_handled,
            total_logs,
            "All malformed logs should be processed or handled gracefully",
        )

        print(
            f"✅ Handled {errors_handled} malformed logs gracefully, processed {successful_processing} successfully"
        )
    ...
    def test_high_volume_log_processing_integration(self):
        ...
        logs_per_second = (
            len(high_volume_logs) / processing_time if processing_time > 0 else 0
        )

        # Assert: Performance should meet minimum requirements
        self.assertLess(
            processing_time, 30.0, "Should process 1000 logs in under 30 seconds"
        )
        self.assertGreater(
            logs_per_second, 10, "Should process at least 10 logs per second"
        )
        self.assertGreater(
            processed_count,
            len(high_volume_logs) * 0.8,
            "Should successfully process at least 80% of logs",
        )

        print(
            f"✅ Processed {processed_count} logs in {processing_time:.2f}s ({logs_per_second:.1f} logs/sec)"
        )
    ...
    def test_component_failure_recovery_in_pipeline(self):
        ...
        with patch.object(
            self.components["pattern_detector"],
            "detect_patterns",
            ...
            for log_entry in test_logs:
                try:
                    parsed_data = self.components["log_parser"].parse_log_line(
                        log_entry
                    )
                    if parsed_data:
                        processed_data = self.components[
                            "log_processor"
                        ].process_log_entry(parsed_data)
                        if processed_data:
                            processed_without_pattern_detection += 1

                            # Pipeline should continue even if pattern detection fails
                            try:
                                self.components["pattern_detector"].detect_patterns(
                                    [processed_data]
                                )
                            except Exception:
                    ...
                    self.fail(
                        f"Pipeline should continue despite pattern detector failure: {e}"
                    )

            # Assert: Basic log processing should continue
            self.assertGreater(
                processed_without_pattern_detection,
                0,
    ...
    def test_threat_escalation_integration(self):
        ...
        self.assertGreater(
            len(high_severity_threats), 0, "Should identify high-severity threats"
        )
        ...
        print(
            f"✅ Identified {len(high_severity_threats)} high-severity threats for escalation"
        )
...
class TestConfigurationSystemIntegration(BaseIntegrationTest):
    ...
    def test_encrypted_config_loading_integration(self):
        ...
        test_config = {
            "service": {"check_interval": 30, "log_file_path": "/test/path/access.log"},
            "alert_system": {
                "email": {"enabled": True, "smtp_server": "test.smtp.com"}
            },
        }
