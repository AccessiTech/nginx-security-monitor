...
class TestCryptoIntegration(BaseIntegrationTest):
    ...
    def setUp(self):
        ...
        self.encrypted_config_file = os.path.join(
            self.crypto_test_dir, "test_config.enc"
        )
        ...
        self.test_config = {
            ...
            "network": {"listen_port": 8080, "max_connections": 1000},
        }
    ...
    def test_end_to_end_encryption_integration(self):
        ...
        crypto_utils.configure(
            {"keys_directory": self.keys_dir, "default_key_file": self.key_file}
        )
        ...
        with open(self.config_file, "r") as f1, open(
            self.encrypted_config_file, "r"
        ) as f2:
        ...
        loaded_config = config_manager.load_encrypted_config(
            self.encrypted_config_file, key=self.test_key
        )

        # Assert: Loaded config should match original
        self.assertEqual(
            loaded_config["service"]["name"], self.test_config["service"]["name"]
        )
        self.assertEqual(
            loaded_config["security"]["enable_ip_blocking"],
        ...
        alert_manager.configure(
            {"credentials_file": self.encrypted_secrets_file, "encrypted": True}
        )
    ...
    def test_key_rotation_integration(self):
        ...
        rotated_config = config_manager.load_encrypted_config(rotated_file, key=new_key)

        self.assertEqual(
            rotated_config["service"]["name"], self.test_config["service"]["name"]
        )
        ...
        reloaded_config = config_manager.load_encrypted_config(
            rotated_file, key=new_key
        )

        self.assertEqual(
            reloaded_config["security"]["enable_ip_blocking"],
            self.test_config["security"]["enable_ip_blocking"],
        )
    ...
    def test_crypto_failure_recovery(self):
        ...
        with patch.object(alert_manager, "send_alert") as mock_alert:
            ...
            result = config_manager.load_encrypted_config(
                self.encrypted_config_file, key=wrong_key, allow_fallback=True
            )
            ...
            self.assertEqual(
                result["service"]["name"], self.test_config["service"]["name"]
            )
...
class TestCryptoDataFlow(BaseIntegrationTest):
    ...
    def test_encrypted_config_propagation(self):
        ...
        ) as plain_file, tempfile.NamedTemporaryFile(
            mode="w", suffix=".enc", delete=False
        ) as enc_file:
        ...
        # Encrypt the config
        crypto_utils.encrypt_file(
            input_file=plain_path, output_file=enc_path, key=test_key
        )
        ...
        loaded_config = config_manager.load_encrypted_config(enc_path, key=test_key)
        ...
        self.assertEqual(
            pattern_detector.get_threshold("requests_per_ip_per_minute"), 123
        )

        self.assertEqual(
            pattern_detector.get_threshold("failed_requests_per_minute"), 45
        )
