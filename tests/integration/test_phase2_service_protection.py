...
class TestServiceProtectionIntegration(BaseIntegrationTest):
    ...
    def test_self_monitoring_integration(self):
        ...
        with patch.object(alert_manager, 'send_alert') as mock_alert, \
             patch.object(monitor_service, 'get_memory_usage') as mock_memory, \
             patch.object(monitor_service, 'get_cpu_usage') as mock_cpu:
    ...
    def test_resource_exhaustion_protection(self):
        ...
        with patch.object(monitor_service, 'get_active_connections') as mock_connections, \
             patch.object(monitor_service, 'drop_connection') as mock_drop:
            ...
            self.assertGreaterEqual(mock_drop.call_count, 22)  # 20 over concurrent limit + 1 oversized + 1 timeout
    ...
    def test_file_integrity_monitoring(self):
        ...
        with patch.object(alert_manager, 'send_alert') as mock_alert:
            ...
            self.assertEqual(alert_data['details']['modified_file'], self.test_files['config'])
...
class TestServiceMonitoring(BaseIntegrationTest):
    ...
    def test_service_restart_coordination(self):
        ...
        with patch.object(monitor_service, 'restart_service') as mock_restart, \
             patch.object(alert_manager, 'send_alert') as mock_alert, \
             patch.object(monitor_service, 'get_service_status') as mock_status:
            ...
            service_protection.handle_unhealthy_service("Memory usage exceeded threshold")
