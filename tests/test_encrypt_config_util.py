...
# Import the encrypt_config module for attribute access
import nginx_security_monitor.encrypt_config as encrypt_config
...
class TestEncryptConfigUtility(unittest.TestCase):
    ...
    def test_encrypt_patterns_file_new_key_generation(
        self, mock_remove, mock_manager_class, mock_print, mock_input, mock_getpass
    ):
        test_master_key = "test_key"
        """Test encrypting patterns with new key generation"""
        # Arrange
        with patch("os.environ.get", return_value=test_master_key):
            mock_getpass.return_value = test_master_key
            ...
            with patch(
                ...
                with patch("builtins.open", mock_open()):
                    with patch("json.dump") as mock_json_dump:
                        ...
                        # Only one remove call is expected
    ...
    def test_encrypt_patterns_file_existing_key(
        self, mock_manager_class, mock_print, mock_input, mock_getpass
    ):
        test_master_key = "test_key"
        ...
        with patch("os.environ.get", return_value=test_master_key):
            ...
            mock_input.side_effect = [
                ...
                "",  # No RPM threshold
                "",  # No FPM threshold
                "",  # Output file prompt (add extra to avoid StopIteration)
                "",  # Extra in case of additional prompt
            ]
