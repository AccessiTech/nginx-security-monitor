        ...
        with patch("os.makedirs") as mock_makedirs:
            with patch("builtins.open", mock_open()) as mock_file:
                with patch("os.chmod") as mock_chmod:
                    with patch(
                        "nginx_security_monitor.config_schema.logger"
                    ) as mock_logger:
                        # Act
                        save_schema_to_file()
                        ...
                        mock_logger.info.assert_called_once_with(
                            f"Schema saved to {default_path}"
                        )
...
"""
    ...
    def test_main_block_direct_execution(self):
        ...
        # Use current working directory for compatibility with CI
        result = subprocess.run(
        ...
        )  # 0 = success, 1 = permission error (acceptable)
                    ...
                    if isinstance(value, dict) and "__type" in value:
                        ...
                        if field_type == "string":
                            self.assertIsInstance(
                                default_value, str, f"Type mismatch in {path}.{key}"
                            )
                        elif field_type == "integer":
                            self.assertIsInstance(
                                default_value, int, f"Type mismatch in {path}.{key}"
                            )
                        elif field_type == "boolean":
                            self.assertIsInstance(
                                default_value, bool, f"Type mismatch in {path}.{key}"
                            )
                        elif field_type == "array":
                            self.assertIsInstance(
                                default_value, list, f"Type mismatch in {path}.{key}"
                            )
                        elif field_type == "object" and default_value is not None:
                            self.assertIsInstance(
                                default_value, dict, f"Type mismatch in {path}.{key}"
                            )
                        elif field_type == "number":
                            self.assertIsInstance(
                                default_value,
                                (int, float),
                                f"Type mismatch in {path}.{key}",
                            )
                    else:
