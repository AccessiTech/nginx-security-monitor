...
class TestFail2BanIntegration(unittest.TestCase):
    ...
    def test_is_available_success(self, mock_run):
        ...
        mock_run.assert_called_with(
            ["fail2ban-client", "ping"], capture_output=True, text=True, timeout=5
        )
...
`- Actions
   ...
   `- Banned IP list:	192.168.1.100 192.168.1.101"""
    ...
    @patch("os.path.isdir")
    def test_monitor_jail_files_with_files_and_dirs(
        self, mock_isdir, mock_isfile, mock_exists
    ):
        """Test monitoring jail files with both files and directories."""
        # Setup file structure - only one path exists in jail_files
        self.integration.jail_files = [
...
""",
    ...
    def test_check_jail_file_weak_config(self, mock_exists, mock_file):
        ...
        self.assertTrue(
            any("weak ban time" in str(threat).lower() for threat in threats)
        )
...
class TestOSSECIntegration(unittest.TestCase):
    ...
    def test_get_recent_alerts(self):
        ...
        with tempfile.NamedTemporaryFile(mode="w", delete=False) as f:
            f.write(
                """2025 Jul 19 14:30:25 test-server->ossec-server
...
"""
    ...
    def test_add_custom_rule_dir_not_exist(self, mock_exists):
        ...
        rule_content = (
            "<rule id='100001' level='10'><description>Test</description></rule>"
        )
...
class TestWazuhIntegration(unittest.TestCase):
    ...
    @patch("syslog.openlog")
    @patch("syslog.closelog")
    def test_send_custom_event_with_priority_mapping(
        self, mock_closelog, mock_openlog, mock_syslog
    ):
        """Test sending custom event with different priority levels."""
        event_data = {"message": "Low priority alert", "priority": "low"}
...
class TestModSecurityIntegration(unittest.TestCase):
    ...
    def setUp(self):
        self.config = {"modsecurity_log": "/var/log/modsec_audit.log"}
...
"""
        ...
        with patch("builtins.open", mock_open(read_data=jail_content)):
            ...
            disabled_jail_threats = [
                t for t in threats if t["type"] == "Disabled Critical Jail"
            ]
...
"""
            ...
            with patch("builtins.open", mock_open(read_data=alert_content)):
                with patch("os.path.exists", return_value=True):
                    ...
                    self.assertEqual(
                        len(alerts), 2
                    )  # Check first alert has src_ip parsed
...
"""
        ...
        with patch("builtins.open", mock_open(read_data=log_content)):
            blocks = integration._parse_modsec_log(
                "/var/log/modsec_audit.log", hours=24
            )
    ...
    def test_ossec_add_custom_rule_creates_file_if_not_exists(self):
        ...
        with patch("os.path.exists") as mock_exists:
            mock_exists.side_effect = (
                lambda path: "/rules" not in path
            )  # Rules dir exists, file doesn't
            with patch("builtins.open", mock_open()) as mock_file:
                with patch("os.makedirs"):
                    result = integration.add_custom_rule(
...
"""
    ...
    def test_security_integration_manager_availability_check_exception(self):
        ...
        config = {"fail2ban": {}, "ossec": {}}

        # Patch at the module level where classes are used
        with patch(
            "nginx_security_monitor.security_integrations.Fail2BanIntegration"
        ) as MockFail2Ban:
            with patch(
                "nginx_security_monitor.security_integrations.OSSECIntegration"
            ) as MockOSSEC:
                # Make fail2ban is_available raise exception
                mock_fail2ban = Mock()
                mock_fail2ban.is_available.side_effect = Exception(
                    "Fail2ban check error"
                )
                ...
                self.assertFalse(
                    manager.available_integrations["fail2ban"]
                )  # Should be False due to exception
                self.assertTrue(
                    manager.available_integrations["ossec"]
    ...
    def test_security_integration_manager_get_status_with_ossec_details(self):
        ...
        with patch(
            ...
            self.assertEqual(
                status["integration_details"]["ossec"]["recent_alerts_count"], 3
            )
            self.assertEqual(
                status["integration_details"]["ossec"]["high_severity_alerts"], 2
            )
    ...
    def test_security_integration_manager_get_status_with_suricata_details(self):
        ...
        with patch(
            "nginx_security_monitor.security_integrations.SuricataIntegration"
        ) as MockSuricata:
            mock_suricata = Mock()
            ...
            self.assertEqual(
                status["integration_details"]["suricata"]["recent_alerts_count"], 3
            )
            self.assertEqual(
                status["integration_details"]["suricata"]["critical_alerts"], 3
            )  # severity >= 1
    ...
    def test_security_integration_manager_get_status_with_modsecurity_details(self):
        ...
        with patch(
            "nginx_security_monitor.security_integrations.ModSecurityIntegration"
        ) as MockModSec:
            mock_modsec = Mock()
            ...
            self.assertEqual(
                status["integration_details"]["modsecurity"]["recent_blocks_count"], 2
            )
